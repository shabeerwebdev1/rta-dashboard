...
const TradePage: React.FC = () => {
  ...
  useEffect(() => {
    setPageTitle(t("sidebar.addtrade"));
    const allData = generateWhitelistData(50);
    const tradeData = allData.filter((item) => item.type === "Trade");
    setTableData(tradeData);
  ...
  const handleDownloadCsv = () => {
    ...
    modal.confirm({
      ...
      onOk() {
        try {
          exportToCsv(filteredData, "trade_export.csv");
  ...
  const handleFormSubmit = (values: any) => {
    ...
    setTimeout(() => {
      const newRecord: WhitelistRecord = {
        ...
        licenseNumber: values.tradeLicenseNumber || "N/A",
        date: values.endDate,
        status: "Active",
        type: "Trade",
      ...
      message.success(t("messages.tradeAdded"));
    }, 1500);
  };
