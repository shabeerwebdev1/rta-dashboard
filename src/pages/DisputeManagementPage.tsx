...
const DisputeManagementPage: React.FC = () => {
  ...
  // Fetch disputes with filter for pending disputes
  const { data, currentData, isLoading, isFetching, refetch } = useGetDisputesQuery(
    {
      ...apiParams,
      // When "Show Pending Disputes" is enabled
      ...(showPendingDisputes && userRoleGUID && { 
        assignedToRole: userRoleGUID, // Filter by user's roleGUID
        dispute_Status: [1, 4] // Pending (1) and In Review (4) statuses
      }),
    },
    { refetchOnMountOrArgChange: true },
  );
  ...
  const handleShowPendingDisputes = () => {
    ...
    if (showPendingDisputes) {
      ...
      // Also clear any column filter for assignedToRole
      clearFilter("column", "assignedToRole");
    } else {
      // Apply the filter
  ...
  // Function to check if row is assigned to current user's role
  const isRowAssignedToUserRole = (record: any) => {
    if (!userRoleGUID) return false;
    return record.assignedToRole === userRoleGUID;
  };
  ...
  // Filter data locally as a fallback
  const filteredData = useMemo(() => {
    if (showPendingDisputes && userRoleGUID && data?.data) {
      return data.data.filter((record: any) => 
        record.assignedToRole === userRoleGUID && 
        (record.dispute_Status === 1 || record.dispute_Status === 4)
      );
    }
    return data?.data || [];
