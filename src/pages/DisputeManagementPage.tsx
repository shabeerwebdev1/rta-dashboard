...
const DisputeManagementPage: React.FC = () => {
  ...
  const handleFineHover = useCallback(
    async (record: any) => {
      ...
      try {
        ...
        const result = await triggerGetDisputeById({ type: "vehicle", id: disputeCode }).unwrap();
  ...
  useEffect(() => {
    ...
    const loadBatch = async () => {
      const results = await Promise.all(
        unresolvedCodes.map(async (code: string) => {
          try {
            const result = await triggerGetDisputeById({ type: "vehicle", id: code }).unwrap();
            return { code, data: result?.data || null };
          } catch {
            return { code, data: null };
          }
  ...
  const handleModalOpen = async (mode: "add" | "edit", record?: any) => {
    ...
    if (mode === "edit" && record) {
      try {
        const result = await triggerGetDisputeById({ type: "vehicle", id: record.disputeCode }).unwrap();
        if (result.data) {
          let fileList: any[] = [];
          if (result.data.evidencePath) {
            const files = result.data.evidencePath.split(";");
  ...
  const handleFormSubmit = async (values: any) => {
    try {
      ...
      if (modalMode === "add") {
        response = await addDispute({ type: "vehicle", body: formData }).unwrap();
      } else {
        formData.append("dispute_Id", selectedRecord.dispute_Id);
        response = await updateDispute(formData).unwrap();
      }
