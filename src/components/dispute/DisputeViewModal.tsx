...
const DisputeViewModal: React.FC<DisputeViewModalProps> = ({ open, onClose, disputeId, onStatusUpdate }) => {
  ...
  // Define role GUIDs as per your specification
  const dcRoleGUID = "efb6ef6a-128b-4dd9-9641-2b04205c8cf8"; // Dispute Coordinator
  const seniorSupervisorRoleGUID = "9e8331a6-3828-421b-9c5d-835f7b6f8710"; // Senior Supervisor
  const supervisorRoleGUID = "137db453-07cc-4218-9ef8-3aa236d9e951"; // Supervisor
  const directorRoleGUID = "6d20d858-1128-4cd2-af7e-e8eb3c4bf887"; // Director
  const managerRoleGUID = "33fa8623-20f8-417a-b655-18da372f18bd"; // Manager

  // All supervisor role GUIDs that can approve/reject
  const supervisorRoleGUIDs = [
    seniorSupervisorRoleGUID,
    supervisorRoleGUID,
    managerRoleGUID,
    directorRoleGUID
  ];

  // Check if user is DC (Dispute Coordinator)
  const isDCRole = user?.roleGUID === dcRoleGUID;
  
  // Check if user is a supervisor (can approve/reject)
  const isSupervisorRole = supervisorRoleGUIDs.includes(user?.roleGUID || "");

  // Define the roles for DC assignment dropdown
  ...
  // Check if dispute has been assigned to a role GUID
  const isDisputeAssignedToRole = useMemo(() => {
    if (!disputeData?.data?.reviews?.length) return false;
    
    // Check the last review to see if it was an assignment
    const lastReview = disputeData.data.reviews[disputeData.data.reviews.length - 1];
    
    // Check if last action was assignment
    if (lastReview.review_Action === 1) {
      // Check if assignedTo is a role GUID (UUID format)
      if (lastReview.assignedTo) {
        const isRoleGUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(lastReview.assignedTo);
        return isRoleGUID;
      }
      
      // Also check if there's a separate assignedToRole field
      if (lastReview.assignedToRole) {
        return true;
      }
    }
    return false;
  }, [disputeData?.data?.reviews]);

  // Alternative: Check if dispute has assignedToRole field in the main dispute object
  const hasAssignedRoleGUID = useMemo(() => {
    // Check if dispute has assignedToRole field (from API)
    if (disputeData?.data?.assignedToRole) {
      return true;
    }
    
    // Check if any review has assignedToRole
    if (disputeData?.data?.reviews?.some((review: any) => review.assignedToRole)) {
      return true;
    }
  ...
  const handleStatusUpdate = async (action: number) => {
    try {
      ...
      // For assignment, send the role GUID, not a specific user
      const payload = {
        dispute_Id: storedDisputeId,
        review_Action: action,
        review_Comments: values.review_Comments,
        assignedTo: values.assignedToRole || "", // Send role GUID
        assignedToRole: values.assignedToRole || "", // Send role GUID as assignedToRole
        action_type: action === 1 ? "Assigned" : action === 2 ? "Approved" : "Rejected",
      };
  ...
  const showFooterForSupervisors = useMemo(() => {
    return isSupervisorRole && isDisputePendingOrInReview && (isDisputeAssignedToRole || hasAssignedRoleGUID);
  }, [isSupervisorRole, isDisputePendingOrInReview, isDisputeAssignedToRole, hasAssignedRoleGUID]);
  ...
  return (
    ...
    >
      <Spin spinning={isLoading || isUpdating || isLoadingLookups}>
        ...
        {dispute && !isDisputeApprovedOrRejected && (isDCRole || showFooterForSupervisors) && (
          <>
            <Divider />
            <Form form={form} layout="vertical">
              <Row gutter={16} align="middle">
                {/* Assignment Dropdown - Only for DC role - Shows roles, not users */}
                {isDCRole && (
                  <Col span={6}>
                    <Form.Item
                      name="assignedToRole"
                      label={<Text strong>{t("form.assignToRole")}</Text>}
                      rules={[
                        {
                          required: true,
                    ...
                    >
                      ...
                      >
                        {assignmentRoles.map((role) => (
                          <Select.Option key={role.roleGUID} value={role.roleGUID}>
                ...
                {/* Comment Box - Show for DC when assigning OR for supervisors when dispute is assigned to role */}
                {(isDCRole || showFooterForSupervisors) && (
                  <Col span={isDCRole ? (showFooterForSupervisors ? 10 : 12) : 16}>
                    <Form.Item
                      name="review_Comments"
                      label={<Text strong>{t("form.comments")}</Text>}
                      style={{ marginBottom: 0 }}
                    ...
                    <div>
                      <div style={{ marginBottom: 8, textAlign: 'center' }}>
                        <Text type="secondary" style={{ fontSize: 12 }}>
                          As a reviewer, please select your decision.
                        </Text>
                      </div>
                      <div>
                        <Button
